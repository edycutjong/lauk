 ...
 * `~/.config/lauk/upload-keystore.jks` and its password in
 * `~/.config/lauk/secrets.env`, per the system contract that credentials live
 * in `~/.config/<tool>/` and never in the tree. They reach Gradle as
 * environment variables:
 *
 *   LAUK_KEYSTORE_PATH, LAUK_KEYSTORE_ALIAS, LAUK_KEYSTORE_PASSWORD
 *
 * IF THEY ARE ABSENT
 *
...
const RELEASE_SIGNING_CONFIG = `
        release {
            storeFile file(System.getenv("LAUK_KEYSTORE_PATH"))
            storePassword System.getenv("LAUK_KEYSTORE_PASSWORD")
            keyAlias System.getenv("LAUK_KEYSTORE_ALIAS")
            keyPassword System.getenv("LAUK_KEYSTORE_PASSWORD")
        }`;
...
function withReleaseSigning(config) {
  return withAppBuildGradle(config, (cfg) => {
    ...
    const hasCredentials =
      !!process.env.LAUK_KEYSTORE_PATH &&
      !!process.env.LAUK_KEYSTORE_ALIAS &&
      !!process.env.LAUK_KEYSTORE_PASSWORD;

    if (!hasCredentials) {
      console.warn(
        '\n⚠️  withReleaseSigning: LAUK_KEYSTORE_* not set — the release build will be\n' +
          '   signed with the DEBUG keystore. It will install and run, but Google Play\n' +
          '   will reject it. Source the credentials with:\n' +
          '     set -a; . ~/.config/lauk/secrets.env; set +a\n',
      );
      return cfg;
    }
