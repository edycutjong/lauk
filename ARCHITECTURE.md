...
flowchart LR
  subgraph Device["Android — Lauk (Expo / RN)"]
    ...
    ACCESS["rc/access.ts\ncanUseDeck · dailyCap · canCheck · paywallOutcome"]
...
onboarding ─▶ plates ─(tile tap)─▶ ceiling sheet ─▶ result (ring + 3 cards) ─(tap card)─▶ cue ─▶ faces ─▶ plates
                 ...
                 ├─ paywall can't open (offerings error) ──▶ "Store unavailable" line under that deck
...
| `app/src/rc/access.ts`                  | entitlement math over `CustomerInfo.entitlements.active` + `paywallOutcome` (pure, tested)                                                                                                                                                                              |
